...
bool Pawn::valid_capture(const Tile &pos) const {
	std::cout << "TODO: account for en passant\n";
	const int row_diff = pos.row - get_row();
	const int col_abs_diff = abs(pos.col - get_col());
...
bool Pawn::valid_placement(const Tile &pos) const {
	const int row_diff = pos.row - get_row();
	const int col_diff = pos.col - get_col();
